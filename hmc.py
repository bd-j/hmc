...
class BasicHMC(object):
    ...
    def trajectory(self, theta0, model, epsilon, length, lnP0 = None, grad0 = None):
        ...
        theta = theta0.copy() 
        p0 = np.random.normal(0,1, len(theta0)) #random initial momenta
        if grad0 is None:
            grad0 = -model.lnprob_grad(theta0) #gradient in U at initial position, negative of gradient lnP
        if lnP0 is None:
    ...
    def find_reasonable_epsilon(self, theta0, model, epsilon_guess =1):
        ...
        while condition:
            p = p0.copy()
            epsilon = 2.**a * epsilon
            thetaprime, pprime, gradprime = self.leapfrog(theta0.copy(), p, epsilon, grad0, model, check_oob = True)
            ...
            if alpha is 0.0: raise ValueError('alpha is 0')
        return epsilon

    def reset(self):
        self.traj_num = 0 #use this to keep track of the trajectory number
                          #within the trajectory (for storage)
        self.H_t = 0
        self.logepsbar = 0
        self.delta = 0.65
